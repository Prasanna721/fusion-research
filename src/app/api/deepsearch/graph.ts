...
import { initChatModel } from './chatModelFactory';
...
async function generateReportPlan(
  ...
  const structuredQueryLlm = writerModel.withStructuredOutput(
    GroundedQueriesSchema
  );
  ...
  const paper_type = queryResults.paper_type;

  const queryList = queryResults.queries.map((q) => q.search_query);
  const context = Prompts.reportPlannerContext(queryList);
...
const sectionBuilder = new StateGraph({
  ...
  .addNode('write_section', writeSectionNode);
