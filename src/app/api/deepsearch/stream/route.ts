...
export async function GET(request: Request) {
  ...
  if (!url) {
    return NextResponse.json(
      { error: 'url parameter is required for the research topic' },
      { status: 400 }
    );
  }
    ...
    async start(controller) {
      ...
      try {
        ...
        sendEvent('system_message', {
          ...
          message: Mustache.render(EVENT_TEMPLATE_INIT_EXT, { url: url }),
        });
        ...
        if (contentType.success && contentType.isDocumentUrl) {
          sendEvent('system_message', {
            ...
            provider: 'mistral',
            message: EVENT_TEMPLATE_START_EXT_MISTRAL,
          });
          ...
          sendEvent('system_message', {
            ...
            provider: 'firecrawl',
            message: EVENT_TEMPLATE_START_EXT_FIRECRAWL,
          });
